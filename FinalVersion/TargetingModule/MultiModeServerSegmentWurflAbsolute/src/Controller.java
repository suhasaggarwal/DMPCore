...
public class Controller extends HttpServlet {
	...
	private String[][] campInfo(String siteID, String chnlID, String country,
		...
		boolean absoluteControlModeEnabled = true;
		...
		if (fingerprintSegments.length > 1)
			fingerprintId = fingerprintSegments[0] + "."
					+ fingerprintSegments[1] +
					"." + fingerprintSegments[2];
		ArrayList targetedCampaigns = new ArrayList();

		if (absoluteControlModeEnabled == true) {
			...
			for (SearchHit hit : fingerprintDetails) {
				...
				for (int i = 0; i < targetedCampaigns.size(); i++) {
					campaignId1 = (String) targetedCampaigns.get(i);
				...
				if (campaignId1 != null && !campaignId1.isEmpty()) {
					for (int x1 = 0; x1 < allcamplength; x1++) {
						...
						if (campaignId1.trim().equalsIgnoreCase(
							...
							abscampinfo[tracker][0] = allcampinfo[x1][0];
							System.out.println(allcampinfo[x1][0]);
							abscampinfo[tracker][1] = allcampinfo[x1][1];
							System.out.println(allcampinfo[x1][1]);
						    abscampinfo[tracker][2] = allcampinfo[x1][2];
							abscampinfo[tracker][3] = allcampinfo[x1][3];
							abscampinfo[tracker][4] = allcampinfo[x1][4];
							abscampinfo[tracker][5] = allcampinfo[x1][5];
							abscampinfo[tracker][6] = allcampinfo[x1][6];
							abscampinfo[tracker][7] = allcampinfo[x1][7];
							abscampinfo[tracker][8] = allcampinfo[x1][8];
							abscampinfo[tracker][9] = allcampinfo[x1][9];
							abscampinfo[tracker][10] = allcampinfo[x1][10];
							abscampinfo[tracker][11] = allcampinfo[x1][11];
							abscampinfo[tracker][12] = allcampinfo[x1][12];
							abscampinfo[tracker][13] = allcampinfo[x1][13];
							abscampinfo[tracker][14] = allcampinfo[x1][14];
							tracker++;
		...
		if (audienceSegmentEnabled == true) {
			...
			if (randomNumber < 20) {
				...
				String campaignId = null;
				SearchHit[] fingerprintDetails = ElasticSearchAPIs
						.searchDocumentFingerprintIds(client, "testdata2",
				...
				int tracker = 0;
				segmentcampinfo = new String[allcamplength][21];
				int count1 = 0;
				...
				campaignDetailsTopPriority = ElasticSearchAPIs
						.searchDocumentCampaignIds(client,
								"campaignkeywordindex3", "core2", "keywords",
								latestaudienceSegment);
				...
				for (int i = 0; i < validCampaigns3.size(); i++) {
					...
					if (campaignId1 != null && !campaignId1.isEmpty()) {
						for (int x1 = 0; x1 < allcamplength; x1++) {
							...
							if (campaignId1.trim().equalsIgnoreCase(
									(allcampinfo[x1][1]).trim())) {
								segmentcampinfo[tracker][0] = allcampinfo[x1][0];
								segmentcampinfo[tracker][1] = allcampinfo[x1][1];
								segmentcampinfo[tracker][2] = allcampinfo[x1][2];
								segmentcampinfo[tracker][3] = allcampinfo[x1][3];
								segmentcampinfo[tracker][4] = allcampinfo[x1][4];
								segmentcampinfo[tracker][5] = allcampinfo[x1][5];
								segmentcampinfo[tracker][6] = allcampinfo[x1][6];
								segmentcampinfo[tracker][7] = allcampinfo[x1][7];
								segmentcampinfo[tracker][8] = allcampinfo[x1][8];
								segmentcampinfo[tracker][9] = allcampinfo[x1][9];
								segmentcampinfo[tracker][10] = allcampinfo[x1][10];
								segmentcampinfo[tracker][11] = allcampinfo[x1][11];
								segmentcampinfo[tracker][12] = allcampinfo[x1][12];
								segmentcampinfo[tracker][13] = allcampinfo[x1][13];
								segmentcampinfo[tracker][14] = allcampinfo[x1][14];
								tracker++;

							}

							System.out.println("All three matching campaigns:"
									+ campaignId1);
				...
				for (int i = 0; i < validCampaigns2.size(); i++) {
					if (validCampaigns2 != null && !validCampaigns2.isEmpty())
						campaignId1 = (String) validCampaigns2.get(i);
					...
					if (campaignId1 != null && !campaignId1.isEmpty()) {
						...
						// Convert allcampinfo to specific campaign data
						// returned by
						// elastic search, it can give a mixture of campaignIds
						// with
						// geo targeting enabled or disabled.
				...
				for (int i = 0; i < validCampaigns1.size(); i++) {
					if (validCampaigns1 != null && !validCampaigns1.isEmpty())
						campaignId1 = (String) validCampaigns1.get(i);
				...
				if (geoEnabled == false && deviceTargEnabled == false)
				{
					
				    if(tracker > 0)
					return segmentcampinfo;
				}
				...
				HashSet<String> latestfrequentaudienceSegments = CategoryAPIs
						.getLatestCategory(fingerprintDetails);
				...
				for (SearchHit hit : fingerprintDetails) {
					...
					audienceSegmentValue = audienceSegmentValue.trim();
					if (audienceSegmentValue != null
							&& !audienceSegmentValue.isEmpty()) {
						if (audienceSegmentFrequent
								.containsKey(audienceSegmentValue) == false) {
							audienceSegmentFrequent
									.put(audienceSegmentValue, 1);
							System.out.println(result.get("request_time")
									+ "\n");
							System.out
									.println("Adding Value First Time" + "\n");
						} else {

							count = audienceSegmentFrequent
									.get(audienceSegmentValue);
							audienceSegmentFrequent.put(audienceSegmentValue,
									count + 1);
							value = count + 1;
							System.out.println(result.get("request_time")
									+ "\n");
							System.out
									.println("Added Count to field count:"
											+ value + ":"
											+ audienceSegmentValue + "\n");
				...
				while (it.hasNext()) {
					...
					set.add(new Frequency((String) pair.getKey(),
				...
				for (int i = 0; i < mostFrequentSegments.size(); i++) {
					if (latestfrequentaudienceSegments
							.contains(mostFrequentSegments.get(i)))
						latestFrequentCategories.add(mostFrequentSegments
								.get(i));
					mostFrequentCategories.add(mostFrequentSegments.get(i));

				}

				int index = (int) (Math.random() * (latestFrequentCategories
						.size()));
				if (latestFrequentCategories != null
						&& !latestFrequentCategories.isEmpty())
					latestaudienceSegment = (String) latestFrequentCategories
							.get(index);
				...
				// Display campaign that is top priority i.e whose category is
				// most frequent and latest.
				...
				if (mostFrequentCategories != null
						&& !mostFrequentCategories.isEmpty())
					mostFrequentAudienceSegment = (String) mostFrequentCategories
							.get(index);

				campaignDetailsMostFrequent = ElasticSearchAPIs
						.searchDocumentCampaignIds(client,
								"campaignkeywordindex3", "core2", "keywords",
								mostFrequentAudienceSegment);
				...
				if (campaignDetailsMostFrequent != null) {
					for (SearchHit hit : campaignDetailsMostFrequent) {
						...
						System.out.println("frequentcampaign:"
								+ frequentCampaign);
				...
				if (latestfrequentaudienceSegments
						.contains(latestaudienceSegment)) {
					for (int i = 0; i < topCampaign.size(); i++) {
						if (topCampaign != null && !topCampaign.isEmpty())
							campaignId1 = (String) topCampaign.get(i);
						...
						if (campaignId1 != null && !campaignId1.isEmpty()) {
							for (int x1 = 0; x1 < allcamplength; x1++) {
								...
								System.out.println("Top Campaign:"
										+ campaignId1);
					...
					if (campaignId1 != null && !campaignId1.isEmpty()) {
						for (int x1 = 0; x1 < allcamplength; x1++) {
							...
							if (campaignId1.trim().equalsIgnoreCase(
								...
								tracker++;
								System.out
										.println("All three Matching Categories:"
												+ campaignId1);
				...
				for (int i = 0; i < frequentCampaign.size(); i++) {
					if (frequentCampaign != null && !frequentCampaign.isEmpty())
						campaignId1 = (String) frequentCampaign.get(i);
					...
					if (campaignId1 != null && !campaignId1.isEmpty()) {
						for (int x1 = 0; x1 < allcamplength; x1++) {
							...
							System.out.println("Frequent Campaign:"
									+ campaignId1);
				...
				if (campaignId1 != null && !campaignId1.isEmpty()) {
					for (int x1 = 0; x1 < allcamplength; x1++) {
						...
						System.out
								.println("1 Matching Campaign:" + campaignId1);
		...
		geocampinfo = new String[allcamplength][21];
		...
		for (int x = 0; x < allcamplength; x++) {
			...
			{ // For audience segmentation , call will come for channel id,
				...
				if (allcampinfo[x][3].equals("1")) // if it is geo targeted
				{
														...
														// - look match
					{
						...
						if (matched) {
							...
							// allcampinfo[x][1]);
							geocampinfo[mcicnt][0] = allcampinfo[x][0];
							geocampinfo[mcicnt][1] = allcampinfo[x][1];
							geocampinfo[mcicnt][2] = allcampinfo[x][2];
							geocampinfo[mcicnt][3] = allcampinfo[x][3];
							geocampinfo[mcicnt][4] = allcampinfo[x][4];
							geocampinfo[mcicnt][5] = allcampinfo[x][5];
							geocampinfo[mcicnt][6] = allcampinfo[x][6];
							geocampinfo[mcicnt][7] = allcampinfo[x][7];
							geocampinfo[mcicnt][8] = allcampinfo[x][8];
							geocampinfo[mcicnt][9] = allcampinfo[x][9];
							geocampinfo[mcicnt][10] = allcampinfo[x][10];
							geocampinfo[mcicnt][11] = allcampinfo[x][11];

							geocampinfo[mcicnt][12] = country;
							geocampinfo[mcicnt][13] = state;
							geocampinfo[mcicnt][14] = city;
							mcicnt++;
						} else
							continue;
				...
				else // if it is not geo targeted
				{
					...
					mycampinfo[mcicnt2][0] = allcampinfo[x][0];
					mycampinfo[mcicnt2][1] = allcampinfo[x][1];
				    mycampinfo[mcicnt2][2] = allcampinfo[x][2];
					mycampinfo[mcicnt2][3] = allcampinfo[x][3];
					mycampinfo[mcicnt2][4] = allcampinfo[x][4];
					mycampinfo[mcicnt2][5] = allcampinfo[x][5];
					mycampinfo[mcicnt2][6] = allcampinfo[x][6];
					mycampinfo[mcicnt2][7] = allcampinfo[x][7];
					mycampinfo[mcicnt2][8] = allcampinfo[x][8];
					mycampinfo[mcicnt2][9] = allcampinfo[x][9];
					mycampinfo[mcicnt2][10] = allcampinfo[x][10];
					mycampinfo[mcicnt2][11] = allcampinfo[x][11];

					mycampinfo[mcicnt2][12] = country;
					mycampinfo[mcicnt2][13] = state;
					mycampinfo[mcicnt2][14] = city;
		...
		if (audienceSegmentEnabled == false && deviceTargEnabled == false && geoEnabled == true){
			if(mcicnt > 0)
			return geocampinfo;
		}
		
		
		if (audienceSegmentEnabled == false && geoEnabled == false && deviceTargEnabled == false ){
			if(mcicnt2 > 0)
			return mycampinfo;
			}
		...
		if (userAgent.toLowerCase().contains("msie")) {
			...
			browser1 = "safari";
				...
				|| userAgent.toLowerCase().contains("opera")) {

			browser1 = "opera";
		} else if (userAgent.toLowerCase().contains("chrome")) {
			browser1 = "chrome";
		} else if ((userAgent.toLowerCase().contains("mozilla"))
				|| userAgent.toLowerCase().contains("netscape")) {
		...
		for (int x = 0; x < allcamplength; x++) {
			...
			if (userAgent != null && userAgent != "") {
				...
				if (os1.toLowerCase()
						.contains(allcampinfo[x][15].toLowerCase()))
					os = 1;

				if (browser1.toLowerCase().contains(
						allcampinfo[x][16].toLowerCase()))
					browser = 1;

				if (os1.toLowerCase()
						.contains(allcampinfo[x][17].toLowerCase()))
					os_version = 1;
				...
				if (Integer.parseInt(allcampinfo[x][18]) == 1) {
					...
					if (os == 1) {
						...
						devicecampinfo[mcicnt1][0] = allcampinfo[x][0];
						devicecampinfo[mcicnt1][1] = allcampinfo[x][1];
						devicecampinfo[mcicnt1][2] = allcampinfo[x][2];
						devicecampinfo[mcicnt1][3] = allcampinfo[x][3];
						devicecampinfo[mcicnt1][4] = allcampinfo[x][4];
						devicecampinfo[mcicnt1][5] = allcampinfo[x][5];
						devicecampinfo[mcicnt1][6] = allcampinfo[x][6];
						devicecampinfo[mcicnt1][7] = allcampinfo[x][7];
						devicecampinfo[mcicnt1][8] = allcampinfo[x][8];
						devicecampinfo[mcicnt1][9] = allcampinfo[x][9];
						devicecampinfo[mcicnt1][10] = allcampinfo[x][10];
						devicecampinfo[mcicnt1][11] = allcampinfo[x][11];
						mcicnt1++;
						System.out.println("CampInfo:"
								+ devicecampinfo[mcicnt1][0]
								+ devicecampinfo[mcicnt1][1]
								+ devicecampinfo[mcicnt1][2]
								+ devicecampinfo[mcicnt1][3]);
						target = 1;
			...
			if (audienceSegmentEnabled == false && geoEnabled == false && deviceTargEnabled == true ){
			if(mcicnt1 > 0)
			return devicecampinfo;
			}
		
		
			}

		ArrayList<String> segmentCampaign = new ArrayList<String>();
		ArrayList<String> geoCampaign = new ArrayList<String>();
		ArrayList<String> deviceCampaign = new ArrayList<String>();

		for (int i = 0; i < segmentcampinfo.length; i++) {
			segmentCampaign.add(segmentcampinfo[i][1]);
		}

		for (int i = 0; i < geocampinfo.length; i++) {
			geoCampaign.add(geocampinfo[i][1]);
		}

		for (int i = 0; i < devicecampinfo.length; i++) {
			deviceCampaign.add(devicecampinfo[i][1]);
		}

		ArrayList<String> segGeoCampaign = new ArrayList<String>(
				segmentCampaign);
		int counter = 0;

		if (audienceSegmentEnabled == true && geoEnabled == true) {

			segGeoCampaign.retainAll(geoCampaign);
			for (int i = 0; i < segGeoCampaign.size(); i++) {

				campaignIds = (String) segGeoCampaign.get(i);

				if (campaignIds != null && !campaignIds.isEmpty()) {
					for (int x1 = 0; x1 < allcamplength; x1++) {

						if (campaignIds.trim().equalsIgnoreCase(
								(segGeocampinfo[x1][1]).trim())) {
							segGeocampinfo[counter][0] = segmentcampinfo[x1][0];
							segGeocampinfo[counter][1] = segmentcampinfo[x1][1];
							segGeocampinfo[counter][2] = segmentcampinfo[x1][2];
							segGeocampinfo[counter][3] = segmentcampinfo[x1][3];
							segGeocampinfo[counter][4] = segmentcampinfo[x1][4];
							segGeocampinfo[counter][5] = segmentcampinfo[x1][5];
							segGeocampinfo[counter][6] = segmentcampinfo[x1][6];
							segGeocampinfo[counter][7] = segmentcampinfo[x1][7];
							segGeocampinfo[counter][8] = segmentcampinfo[x1][8];
							segGeocampinfo[counter][9] = segmentcampinfo[x1][9];
							segGeocampinfo[counter][10] = segmentcampinfo[x1][10];
							segGeocampinfo[counter][11] = segmentcampinfo[x1][11];
							segGeocampinfo[counter][12] = segmentcampinfo[x1][12];
							segGeocampinfo[counter][13] = segmentcampinfo[x1][13];
							segGeocampinfo[counter][14] = segmentcampinfo[x1][14];
							counter++;
			...
			if(counter > 0)
			return segGeocampinfo;
		}

		ArrayList<String> segDevCampaign = new ArrayList<String>(
				segmentCampaign);

		counter = 0;

		if (audienceSegmentEnabled == true && deviceTargEnabled == true) {

			segDevCampaign.retainAll(deviceCampaign);
			for (int i = 0; i < segDevCampaign.size(); i++) {

				campaignIds = (String) segDevCampaign.get(i);

				if (campaignIds != null && !campaignIds.isEmpty()) {
					for (int x1 = 0; x1 < allcamplength; x1++) {

						if (campaignIds.trim().equalsIgnoreCase(
								(segDevcampinfo[x1][1]).trim())) {
							segDevcampinfo[counter][0] = segmentcampinfo[x1][0];
							segDevcampinfo[counter][1] = segmentcampinfo[x1][1];
							segDevcampinfo[counter][2] = segmentcampinfo[x1][2];
							segDevcampinfo[counter][3] = segmentcampinfo[x1][3];
							segDevcampinfo[counter][4] = segmentcampinfo[x1][4];
							segDevcampinfo[counter][5] = segmentcampinfo[x1][5];
							segDevcampinfo[counter][6] = segmentcampinfo[x1][6];
							segDevcampinfo[counter][7] = segmentcampinfo[x1][7];
							segDevcampinfo[counter][8] = segmentcampinfo[x1][8];
							segDevcampinfo[counter][9] = segmentcampinfo[x1][9];
							segDevcampinfo[counter][10] = segmentcampinfo[x1][10];
							segDevcampinfo[counter][11] = segmentcampinfo[x1][11];
							segDevcampinfo[counter][12] = segmentcampinfo[x1][12];
							segDevcampinfo[counter][13] = segmentcampinfo[x1][13];
							segDevcampinfo[counter][14] = segmentcampinfo[x1][14];
							counter++;
			...
			if(counter > 0)
		    return segDevcampinfo;
		}

		ArrayList<String> geoDevCampaign = new ArrayList<String>(geoCampaign);

		counter = 0;

		if (geoEnabled == true && deviceTargEnabled == true) {

			geoDevCampaign.retainAll(deviceCampaign);
			for (int i = 0; i < geoDevCampaign.size(); i++) {

				campaignIds = (String) geoDevCampaign.get(i);

				if (campaignIds != null && !campaignIds.isEmpty()) {
					for (int x1 = 0; x1 < allcamplength; x1++) {

						if (campaignIds.trim().equalsIgnoreCase(
								(geoDevcampinfo[x1][1]).trim())) {
							geoDevcampinfo[counter][0] = segmentcampinfo[x1][0];
							geoDevcampinfo[counter][1] = segmentcampinfo[x1][1];
							geoDevcampinfo[counter][2] = segmentcampinfo[x1][2];
							geoDevcampinfo[counter][3] = segmentcampinfo[x1][3];
							geoDevcampinfo[counter][4] = segmentcampinfo[x1][4];
							geoDevcampinfo[counter][5] = segmentcampinfo[x1][5];
							geoDevcampinfo[counter][6] = segmentcampinfo[x1][6];
							geoDevcampinfo[counter][7] = segmentcampinfo[x1][7];
							geoDevcampinfo[counter][8] = segmentcampinfo[x1][8];
							geoDevcampinfo[counter][9] = segmentcampinfo[x1][9];
							geoDevcampinfo[counter][10] = segmentcampinfo[x1][10];
							geoDevcampinfo[counter][11] = segmentcampinfo[x1][11];
							geoDevcampinfo[counter][12] = segmentcampinfo[x1][12];
							geoDevcampinfo[counter][13] = segmentcampinfo[x1][13];
							geoDevcampinfo[counter][14] = segmentcampinfo[x1][14];
							counter++;
	...
	private String geocampinfo[][] = null;
	private String segmentcampinfo[][] = null;
	private String abscampinfo[][] = null;
	private String mycampinfo[][] = null;
	private String segGeocampinfo[][] = null;
	private String segDevcampinfo[][] = null;
	private String geoDevcampinfo[][] = null;

	private boolean audienceSegmentEnabled = GlobalConfiguration
			.getBoolean("segmentenabled");
	private boolean geoEnabled = GlobalConfiguration.getBoolean("geoenabled");
	private boolean deviceTargEnabled = GlobalConfiguration
			.getBoolean("deviceTargenabled");
