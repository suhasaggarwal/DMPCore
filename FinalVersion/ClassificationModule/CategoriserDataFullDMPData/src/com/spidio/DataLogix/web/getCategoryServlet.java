...
public class getCategoryServlet extends HttpServlet {
		...
		while (true) {
		    ...
		    for (SearchHit hit : response1.getHits().getHits()) {
		    	...
		    	try {
					...
					if(refurl.contains("voindia")==false && refurl.contains("cornetto")==false && refurl.contains("testurl")==false)
					{	                
					...
					if (audienceSegment == null || audienceSegment == "") {
						...
						if (category != null && category != "")
							IndexCategoriesData.updateDocument(client, "userprofilestore",
									"core2", id, "audience_segment", category);
					...
					if (audienceSegment == null || audienceSegment == "") {
						...
						while ((line = br.readLine()) != null) {
							...
							if (refurl.toLowerCase().contains(keywordcheck.toLowerCase()) == true){
								if(parts1.length== 2)
					 ...
					 * ; URL url; url = new URL(refurl); final InputSource is =
					 * HTMLFetcher.fetch(url).toInputSource();
					 * 
					 * final BoilerpipeSAXInput in = new BoilerpipeSAXInput(is);
					 * final TextDocument doc = in.getTextDocument();
					 * 
					 * TermDocument topiaDoc = new TermDocument(); TermsExtractor
					 * termExtractor = new TermsExtractor(); String text =
					 * ArticleExtractor.INSTANCE.getText(doc); topiaDoc =
					 * termExtractor.extractTerms(text);
					 */// Document doc;
					...
					if (audienceSegment == null || audienceSegment == "") {
						for (int j = 0; j < searchkeyword.length; j++) {
							...
							System.out.println(finalsearchkeyword + "\n");
							if(finalsearchkeyword !=null && !finalsearchkeyword.isEmpty())
						...
						int maxValueInMap = (Collections
								.max(categoryCount.values())); // This will return
																// max value in the
																// Hashmap
	...
	response1 = client.prepareSearchScroll(response1.getScrollId()).setScroll(new TimeValue(60000)).execute().actionGet();
